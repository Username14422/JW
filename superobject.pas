...
begin
  ...
  if {$if defined(NEED_FORMATSETTINGS)}FormatSettings.{$ifend}formatSETTINGS.DateSeparator <> '.' then
  begin
    p := PSOChar(Result);
    while p^ <> #0 do
      if p^ <> SOChar({$if defined(NEED_FORMATSETTINGS)}FormatSettings.{$ifend}formatSETTINGS.DateSeparator) then
      inc(p) else
      begin
        p^ := '.';
        Exit;
